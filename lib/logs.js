...
lib.list = function (includedCompressedLogs, callback) {
  fs.readdir(lib.baseDir, function (err, data) {
    if (!err && data && data.length > 0) {
      ...
      data.forEach(function (fileName) {
        ...
        // Add on the .gz files
        if (fileName.indexOf('.gz') > -1 && includedCompressedLogs) {
        ...
        if (!err && buffer) {
          ...
          fs.open(lib.baseDir + destFile, 'wx', function (err, fileDescriptor) {
            if (!err && fileDescriptor) {
              ...
              fs.writeFile(fileDescriptor, buffer, function (err) {
                if (!err) {
